...
const BINANCE_SUBSCRIPTION_STREAM = "coinhub:subscription:binance:channel"
...
func NewDataCollector(ctx context.Context, redisClient *redis.Client) (*DataCollector, error) {
	binanceConnPool := binance_api.NewWSPool(5)
...
func (dc *DataCollector) AddRedisChannelOption(ctx context.Context, op string, redisChannelArg *binance_define.RedisChannelArg) {
	channelArgsJson, _ := json.MarshalIndent(redisChannelArg, "", "  ")

	field := dataCollectorInstance.apiClient.GetKey(redisChannelArg.TokenPair, redisChannelArg.Channel)

	switch op {
	default:
		log.Printf("Unknown operation: %s", op)
	case "subscribe":
		if dc.apiClient.AddChannelSubscribe(redisChannelArg) {
			logger.Debug("RedisChannelOption, %s: %s", op, string(channelArgsJson))
			for {
				argJson, _ := json.Marshal(redisChannelArg)
				err := dc.redisHelper.HSet(ctx, BINANCE_SUBSCRIBED_CHANNELS, field, argJson)
				if err == nil {
					break
				}
				// 写入失败，重试
	...
	case "unsubscribe":
		logger.Debug("收到取消订阅消息 %s", redisChannelArg)
		if dc.apiClient.AddChannelUnsubscribe(redisChannelArg) {
			logger.Debug("RedisChannelOption, %s: %s", op, string(channelArgsJson))
			for {
				dc.redisHelper.HDel(ctx, BINANCE_SUBSCRIBED_CHANNELS, field)
				_, err := dc.redisHelper.HGet(ctx, BINANCE_SUBSCRIBED_CHANNELS, field)
				if err == redis.Nil {
					break
				}
...
func (dc *DataCollector) RestoreSubscribedChannels(ctx context.Context) error {
	...
	for index, list := range channelMap {
		dc.apiClient.BatchAddChannelSubscribe(index, list)
	}
